...
func TestDouyinUserVideo(t *testing.T) {
	initd()
	client := NewDouyinClient("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36")
	resp, err := client.DouyinUserVideo(nil, "MS4wLjABAAAAz-Nssy-G6nNshJODTK3VpEpjWsH1pMHODDPexGS5K-D6EAo5iASK_qCGRb7M5Rbe")
	if err != nil {
		t.Error(err)
	}
	t.Log(resp)
...
func TestDouyinLiveRoomEnter(t *testing.T) {
	initd()
	client := NewDouyinClient("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36")
	search, err := client.DouyinLiveRoomEnter(nil, "187147137650")
	if err != nil {
		t.Error(err)
	}
	t.Log(search)
